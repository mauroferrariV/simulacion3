...
class QueueSystem:
    ...
    def report(self):
        ...
        queue_probs = [count / self.sim_time for count in self.queue_count[1:]]
        queue_probs.insert(0, 0)  # Prob de 0 clientes en cola
...
def plot_results(results):
    ...
    metrics = ['avg_num_in_system', 'avg_num_in_q', 'avg_time_in_system', 'avg_delay', 'server_util', 'denial_prob']
    metric_names = ['Promedio de clientes en el sistema', 'Promedio de clientes en cola',
                    'Tiempo promedio en sistema', 'Tiempo promedio en cola',
                    'Utilización del servidor', 'Probabilidad de denegación']

    for q_size in queue_sizes:
        fig, axs = plt.subplots(2, 3, figsize=(18, 10))
        fig.suptitle(f'Resultados M/M/1 - Tamaño de cola: {q_size}', fontsize=16)
        ...
        for i, metric in enumerate(metrics):
            ...
            ax.set_xlabel('Tasa de llegada (λ / μ)')
            ax.set_ylabel('Valor')
            ax.grid(True)
    ...
    # Gráfico adicional para mostrar probabilidad de n clientes en cola
    for q_size in queue_sizes:
        plt.figure(figsize=(10, 6))
        for rate in arrival_rates:
            avg_probs = [
                statistics.mean(run['queue_probs'][i] for run in results[(rate, q_size)]) 
                for i in range(len(results[(rate, q_size)][0]['queue_probs']))
            ]
            plt.plot(range(len(avg_probs)), avg_probs, marker='o', label=f'λ/μ = {rate}')
        
        plt.title(f'Probabilidad de n clientes en cola (Q={q_size})')
...
if __name__ == "__main__":
    random.seed(1)
    print("Simulación M/M/1 COMPLETA")

    mean_service = float(input("Ingrese el tiempo promedio de servicio (μ^-1): "))
    num_delays_required = int(input("Cantidad de clientes a simular: "))
    num_runs = int(input("Cantidad de repeticiones por experimento: "))

    print("\nSe usarán por defecto las tasas de llegada: 25%, 50%, 75%, 100%, 125% del servicio.")
    arrival_rates = [0.25, 0.5, 0.75, 1.0, 1.25]
    print("Y los tamaños de cola: 0, 2, 5, 10, 50.")
